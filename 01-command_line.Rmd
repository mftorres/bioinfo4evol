...
First, we will filter out the columns that contain information that is not relevant for us right now. We can use AWK's `print` and field variables to do it. Then, we can look at the *species* columns and remove the records with that field empty (less assume we need coordinate records at the species level, not the genus level).
