...
while IFS='' read -u 9 -r line; do
    ID=$(echo "$line" | cut -f1);
    echo $ID;
    esearch -db nucleotide -query "${ID}[Organism] AND cytochrome[Titl]" | efetch -format fasta >> spiders.fasta
    esearch -db nucleotide -query "${ID}[Organism] AND cytochrome[Titl]" | efetch -format gb >> spiders.gb
